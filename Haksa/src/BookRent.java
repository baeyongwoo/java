...
public class BookRent extends JFrame{
	...
	public BookRent() {
		System.out.println("bookrent 실행");
		...
		JLabel lbldepart=new JLabel("학과");
		lbldepart.setBounds(10, 10, 30, 20); //좌표를 직접 정해서 하는 것
		this.add(lbldepart);
		...
		cbDept.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(cb.getSelectedIndex()==0) { //전체
					...
					query+=" and student.depart='"+dept[1]+"' order by student.id";
				}else if(cb.getSelectedIndex()==2) { //멀티미디어
					query+=" and student.depart='"+dept[2]+"' order by student.id";
				}else if(cb.getSelectedIndex()==3) { //컴퓨터공학
					query+=" and student.depart='"+dept[3]+"' order by student.id";
				}	
